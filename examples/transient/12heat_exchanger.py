"""
Transient concentric-tube heat exchanger example.

This example builds a small counterflow heat exchanger using FullFlow and runs it as a transient model. The hot
...
The example demonstrates several FullFlow features together:
    ...
    4. Lumped fluid volumes with mass and internal-energy storage.
    5. Solid wall thermal nodes with mass and specific heat.
    6. Convection, conduction, and Gnielinski heat-transfer coefficient updates.
    7. Network tracking for pressures, temperatures, flow rates, and heat rates.
...
The model is first solved to steady state with the hot source at its initial
temperature.  That steady solution is then used as the transient initial
condition.  During the transient, a Sequence ramps the hot-source temperature
upward while the coolant source stays fixed.

The fluid nodes are real storage volumes.  Each Volume receives:

    volume
    density
    internal_energy

so FullFlow stores mass and total internal energy during the transient:

    mass = density * volume
    total_internal_energy = mass * internal_energy

The wall nodes are real Solid storage nodes.  Each Solid receives:

    mass
    specific_heat

so FullFlow integrates wall temperature from:

    dT_wall/dt = heat_rate / (mass * specific_heat)
...
# Real storage volumes for each lumped fluid node.  These are the actual
# hydraulic volumes represented by one model segment.
hot_fluid_volume = Aw * L
coolant_volume = Aox * L

# Metal volume represented by each wall node.  There are two wall nodes, each
# covering one modeled segment length.
solid_wall_volume = A_wall_cross_section * L

# Initial guesses for the branch flow rates.  These are intentionally modest
# so the exchanger has visible thermal lag during the transient.  The pressure
# boundary guesses below are chosen to be consistent with these low-flow values.
hot_mass_flow_initial = 0.20
coolant_mass_flow_initial = 0.15

# Transient boundary-temperature command.  The Sequence is inactive during
# SteadyState, so the steady solve initializes the model at the cold value.
hot_source_temperature_initial = 320.0
hot_source_temperature_final = 520.0
hot_source_ramp_start = 0.25
hot_source_ramp_end = 1.25
...
hot_fluid_source_pressure = State(3.500e5)
hot_fluid_source_temperature = State(hot_source_temperature_initial)

def hot_source_temperature_schedule(t):
    """Ramp the fixed hot-source temperature after the initialized state."""

    if t <= hot_source_ramp_start:
        return hot_source_temperature_initial

    if t >= hot_source_ramp_end:
        return hot_source_temperature_final

    ramp_fraction = (t - hot_source_ramp_start) / (hot_source_ramp_end - hot_source_ramp_start)
    return hot_source_temperature_initial + ramp_fraction * (hot_source_temperature_final - hot_source_temperature_initial)


HotSourceTemperatureSequence = Sequence(
    "Hot Source Temperature Sequence",
    HeatExchanger,
    target=hot_fluid_source_temperature,
    function=hot_source_temperature_schedule,
)
...
# property map.
hot_fluid_node1_pressure = State(3.488e5)
hot_fluid_node1_temperature = State(hot_source_temperature_initial)
...
# property map.
hot_fluid_node2_pressure = State(3.476e5)
hot_fluid_node2_temperature = State(hot_source_temperature_initial)
...
HotFluidTube1 = DarcyWeisbach(
    "Hot Fluid Tube 1",
    HeatExchanger,
    mass_flow=hot_mass_flow_initial,
...
HotFluidNode1 = Volume(
    ...
    mass_flow_out=hot_mass_flow_initial
)
...
CoolantTube1 = DarcyWeisbach(
    "Coolant Tube 1",
    HeatExchanger,
    mass_flow=coolant_mass_flow_initial,
...
CoolantNode1 = Volume(
    ...
    mass_flow_out=coolant_mass_flow_initial
)
...
TubeNode1 = Solid(
    ...
    mass=SolidNode1Material.density * solid_wall_volume,
    specific_heat=SolidNode1Material.specific_heat,
)
...
TubeNode2 = Solid(
    ...
    mass=SolidNode2Material.density * solid_wall_volume,
    specific_heat=SolidNode2Material.specific_heat,
)
...
HeatExchanger.track("Hot Fluid Mass Flow 1 [kg/s]", HotFluidTube1.mass_flow)
HeatExchanger.track("Hot Fluid Mass Flow 2 [kg/s]", HotFluidTube2.mass_flow)
HeatExchanger.track("Hot Fluid Mass Flow 3 [kg/s]", HotFluidTube3.mass_flow)

HeatExchanger.track("Coolant Mass Flow 1 [kg/s]", CoolantTube1.mass_flow)
HeatExchanger.track("Coolant Mass Flow 2 [kg/s]", CoolantTube2.mass_flow)
HeatExchanger.track("Coolant Mass Flow 3 [kg/s]", CoolantTube3.mass_flow)

HeatExchanger.track("Hot Fluid Node 1 Heat Rate [W]", HotFluidNode1.heat_rate)
HeatExchanger.track("Hot Fluid Node 2 Heat Rate [W]", HotFluidNode2.heat_rate)
HeatExchanger.track("Coolant Node 1 Heat Rate [W]", CoolantNode1.heat_rate)
HeatExchanger.track("Coolant Node 2 Heat Rate [W]", CoolantNode2.heat_rate)
HeatExchanger.track("Tube Node 1 Heat Rate [W]", TubeNode1.heat_rate)
HeatExchanger.track("Tube Node 2 Heat Rate [W]", TubeNode2.heat_rate)

HeatExchanger.track("Hot Fluid Node 1 Convection Coefficient [W/m2-K]", HotFluid1Solid1Convection.convection_coefficient)
HeatExchanger.track("Coolant Node 2 Convection Coefficient [W/m2-K]", Coolant2Solid1Convection.convection_coefficient)
...
# The steady solve brings every storage node to a consistent initial physical
# state.  The hot-source Sequence is inactive during this solve, so the initial
# source temperature remains hot_source_temperature_initial.
SteadyState(HeatExchanger).solve(
    verbose=True,
    filename=filename,
)
...
# The transient solve starts from the initialized steady-state condition.
# A small timestep is used because the fluid residence time in this short,
# small-diameter heat exchanger is much faster than the wall thermal response.
Transient(HeatExchanger).solve(
    dt=0.01,
    t_final=4.0,
    save_dt=0.02,
    filename=filename,
